...
class PredictionService
{
    public function predictChampionship()
    {
        ...
        // Calculate the simulation weights score
        $simulation = new Simulation();
        $simulationWeightScore = $simulation->calculateSimulationWeightScore($teams);

        // Calculate the points and remaining matches for each team
        $teamPoints = [];
        $remainingMatches = [];
        foreach ($teams as $team) {
            $teamPoints[$team->id] = $team->stats()->sum('points');
        ...
        $totalPoints = array_sum($teamPoints);
        ...
        foreach ($teams as $team) {
            ...
            $championshipPercentage = (($points + ($remaining * $opponentsAveragePoints)) / $totalPoints) * $simulationWeightScore[$team->id];
        ...
        foreach ($teams as $team) {
            $championshipPercentages[$team->id] = ($sum > 0) ? ($championshipPercentages[$team->id] / $sum) * 100 : 0;
        }

        return $championshipPercentages;
    }
